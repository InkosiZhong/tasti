...
class NightStreetOfflineIndex(tasti.Index):
    ...
    def override_target_dnn_cache(self, target_dnn_cache, train_or_test):
        if train_or_test == 'train':
            labels_fp = os.path.join(ROOT_DATA_DIR, 'jackson-town-square-2017-12-14-yolov5s-full.csv')
        else:
            labels_fp = os.path.join(ROOT_DATA_DIR, 'jackson-town-square-2017-12-17-yolov5s-full.csv')
        labels = LabelDataset(
            labels_fp=labels_fp,
            length=len(target_dnn_cache),
            valid_area=(0, 540, 1750, 1080), # xmin, ymin, xmax, ymax
...
class NightStreetOfflineConfig(TASTIConfig):
    def __init__(self):
        super().__init__()
        self.cache_root = 'cache/night_street'
