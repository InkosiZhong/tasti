...
ROOT_DATA_DIR = '/home/inkosizhong/Lab/VideoQuery/datasets/venice-grand-canal'
        
'''
Preprocessing function of a frame before it is passed to the Embedding DNN.
'''
def canal_embedding_dnn_transform_fn(frame):
    frame = cv2.resize(frame, (224, 224))
    frame = torchvision.transforms.functional.to_tensor(frame)
    return frame

def canal_target_dnn_transform_fn(frame):
    frame = torchvision.transforms.functional.to_tensor(frame)
    return frame

class CanalOfflineIndex(tasti.Index):
    ...
    def get_target_dnn_dataset(self, train_or_test):
        if train_or_test == 'train':
            video_fp = os.path.join(ROOT_DATA_DIR, '2018-01-17')
        else:
            video_fp = os.path.join(ROOT_DATA_DIR, '2018-01-20')
        video = VideoDataset(
            video_fp=video_fp,
            transform_fn=canal_target_dnn_transform_fn
        )
        return video
    ...
    def get_embedding_dnn_dataset(self, train_or_test):
        ...
        video = VideoDataset(
            video_fp=video_fp,
            transform_fn=canal_embedding_dnn_transform_fn
        )
        return video
    
    def override_target_dnn_cache(self, target_dnn_cache, train_or_test):
        if train_or_test == 'train':
            labels_fp = os.path.join(ROOT_DATA_DIR, '2018-01-17-yolov5l.csv')
        else:
            labels_fp = os.path.join(ROOT_DATA_DIR, '2018-01-17-yolov5l.csv')
        labels = LabelDataset(
            labels_fp=labels_fp,
...
class CanalOfflineConfig(TASTIConfig):
    def __init__(self):
        super().__init__()
        self.cache_root = 'cache/canal'
...
if __name__ == '__main__':
    config = CanalOfflineConfig()
    index = CanalOfflineIndex(config)
    index.init()
